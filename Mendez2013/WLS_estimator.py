...
"""ESTIMATION USING THE WEIGHTED LEAST SQUARES ESTIMATOR."""
...
def main():
    ...
    WEIGHT_TYPE = "obs"  # Change to "obs/uni" for observations/uniform weights

    # Set number of simulations using the number of rows in the data
    n_sum = data_LOW_SNR.shape[0]  

    # Compute estimators for LOW_SNR
    estimated_x_c_LOW_SNR = fn.estimate_x_c_for_simulations_WLS(data_LOW_SNR, WEIGHT_TYPE, F_LOW_SNR, fs, delta_x, n_pix, D, RON, G, FWHM, INITIAL_GUESS)
    ...
    estimated_x_c_HIGH_SNR = fn.estimate_x_c_for_simulations_WLS(data_HIGH_SNR, WEIGHT_TYPE, F_HIGH_SNR, fs, delta_x, n_pix, D, RON, G, FWHM, INITIAL_GUESS)
    ...
    # Show results
    print(f"Results WLS estimator ({WEIGHT_TYPE} weights)")
    print(f"=====================")
    print(f"Low SNR:")
